...
public class BadgeManagerLevel2 : MonoBehaviour
{
    ...
    IEnumerator Start()
    {
        ...
        if (selectedID == 4)
        {
            ...
            else
            {
                badgeText.text = "YOU RECEIVED:";
                LoadImage(bannerImage, congratulation);
                Debug.Log("BadgeCount is already 2, no increment performed.");
            }
        }
        else
        {
    ...
    {
        int selectedID = PlayerPrefs.GetInt("SelectedId", -1); // Ensure SelectedId is retained only for the current session
        //PlayerPrefs.SetInt("SelectedId", selectedID);

        SceneManager.LoadScene(SceneData.level1result);
    }*/

    public void replayButton2() // not functioning, need to configure
    {
    ...
    {
        SceneManager.LoadScene(SceneData.level2scene2);
    }
